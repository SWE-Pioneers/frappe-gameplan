  ...
  <div class="py-6 transition-shadow" :class="{ ring: highlight }">
    <div class="mb-2 flex items-center text-base text-ink-gray-9">
      ...
      <div class="ml-auto flex items-center space-x-2">
        ...
        <Tooltip v-else :text="__('This is a poll')">
    ...
    submitVote(option) {
      if (this._poll.anonymous) {
        this.$dialog({
          ...
          message: __(
            'This poll is anonymous. Once you vote, you cannot retract your vote. You are voting for "{0}". Continue?',
            [option.title],
          ),
          actions: [
            {
  ...
  computed: {
    ...
    stopTime() {
      ...
      if (this.$dayjs().diff(timestamp, 'day') < 7) {
        return __('Ended {0}', [this.$dayjs(timestamp).fromNow()])
      }
      if (this.$dayjs().diff(timestamp, 'year') < 1) {
        return __('Ended at {0}', [this.$dayjs(timestamp).format('D MMM, h:mm A')])
      }
      return __('Ended at {0}', [this.$dayjs(timestamp).format('D MMM YYYY, h:mm A')])
    },
    _poll() {
