...
class TestGPUserProfile(FrappeTestCase):
	...
	def test_default_bento_cards_use_title_case_select_values(self):
		...
		rendering_values = [card["imageRendering"] for card in response["cards"] if card["type"] == "Image"]

		self.assertTrue(response["is_default"])
		self.assertEqual(card_types, ["Image", "Image", "Text", "Text"])
	...
	def test_default_bento_cards_skip_missing_images(self):
		...
		self.assertTrue(response["is_default"])
		self.assertEqual([card["id"] for card in response["cards"]], ["avatar", "full-name", "bio"])

	def test_first_bento_save_persists_generated_default_cards(self):
		frappe.set_user(self.alice.name)
		profile = get_profile(self.alice.name)
		profile.image = "/files/avatar.png"
		profile.cover_image = "/files/cover.png"
	...
	def test_saved_default_bento_cards_are_not_rehydrated(self):
		frappe.set_user(self.alice.name)
		default_response = get_my_bento_cards()
		original_bio_text = next(card for card in default_response["cards"] if card["id"] == "bio")["text"]
	...
	def test_any_member_can_read_profile_bento_cards(self):
		...
		self.assertTrue(response["is_default"])
		self.assertEqual([card["id"] for card in response["cards"]], ["full-name", "bio"])
	...
	def test_owner_can_save_bento_cards(self):
		...
		self.assertFalse(response["is_default"])
		self.assertEqual(profile.bento_cards[0].type, "Text")
	...
	def test_text_bento_card_must_have_text(self):
		frappe.set_user(self.alice.name)
	...
	def test_image_bento_card_must_have_image(self):
		frappe.set_user(self.alice.name)
		with self.assertRaises(frappe.ValidationError):
			save_my_bento_cards(
				[
	...
	def test_bento_text_card_url_must_use_http_scheme(self):
		frappe.set_user(self.alice.name)
		with self.assertRaises(frappe.ValidationError):
			save_my_bento_cards(
				[
	...
	def test_bento_text_card_url_rejects_control_characters(self):
		frappe.set_user(self.alice.name)
		with self.assertRaises(frappe.ValidationError):
			save_my_bento_cards(
				[
	...
	def test_bento_text_card_url_requires_host(self):
		frappe.set_user(self.alice.name)
		with self.assertRaises(frappe.ValidationError):
			save_my_bento_cards(
				[
