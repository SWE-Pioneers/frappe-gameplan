...
- Cache key shape: `Discussions-${teamId}-${feedType}` for community feeds; bookmarks gets its own key.
...
- `frontend/src/components/DiscussionList.vue` — pin scope branching (`Space` for space lists, `Category` for community lists); cache key includes `teamId` when community-scoped
...
- `frontend/src/components/DiscussionRow.vue`, `frontend/src/components/DiscussionBreadcrumbs.vue` — route params include `teamId`; breadcrumb root is community discussions
...
- update `pinnedDiscussions` cache key to include `teamId` when scoped by community.
