...
Status: done
Commit checkpoint: `8b0546b6` — feat(community): add onboarding and migration support for community scope
Notes:
- **`fetch_from: "project.team"` already existed on `main`.** Task 5a was effectively done before this phase — `gp_discussion.json`'s `team` field already carries `"fetch_from": "project.team"`, so new/edited rows already auto-populate `team`. No JSON change was needed for 5a; verified via `git show main:...`. The redundant `discussion.team = ...` in `move_discussion` was left in place (harmless, per task 5a).
- **Onboarding API now returns a dict** `{team, space}` (was a bare project id string). `onboarding(community, space, icon, emails, is_private=0)` creates the `GP Team` (hook auto-creates `General`), joins the creator via `team.add_member` + `save()`, then creates the user-named space. `Onboarding.vue` updated to collect a community name + space name, persist the community via `communityState.change(team)`, and route to the canonical `Discussions` route.
- **`after_insert` General hook uses a `skip_general_space` flag** (not a built-in `ignore_after_insert` — Frappe has no such flag; `run_method("after_insert")` always fires). The migration sets `team.flags.skip_general_space = True` when creating `Default`. Idempotency rule (skip if any `GP Project` exists in the team) is also enforced as belt-and-suspenders.
- **One combined migration patch** `assign_default_team_to_uncategorized_spaces.py` does both jobs: (1) orphan→`Default` reassignment (only if orphans exist), and (2) the denormalized `team` backfill across all 9 project-linked doctypes, run **unconditionally**. Backfill uses one `UPDATE ... INNER JOIN tabGP Project ... WHERE team IS NULL OR team = ''` per table (idempotent). All 9 doctypes confirmed to have both `team` and `project` fields.
- **`g.py` boot route** now returns `/onboarding` only when there are 0 `GP Project` AND 0 `GP Team`; otherwise `/home`.
- **pin_scope**: `gp_discussion.json` default→`Category`, options→`Category\nSpace`; `pin_discussion()` default arg→`Category`. The hand-maintained frontend union (`doctypes.ts`, `discussions.ts`) was already `'Global' | 'Category' | 'Space'` — left as-is per task 8 (Phase 09 narrows it).
- **Backend test-runner is blocked on this bench** by a Server Script fixture preload (`ServerScriptNotEnabled`) that fails before any test runs — pre-existing, affects every module (reproduced with `test_gp_discussion`), and requires bench-wide `server_script_enabled` which is a guarded security control. Validated all 13 assertions (hook create/idempotent/skip-flag, fetch_from, orphan→Default, no-duplicate-General, backfill→feed-visibility, migration idempotency) via a throwaway `bench execute` script with `frappe.db.rollback()` — all pass. The committed tests live in `test_gp_team.py` and `gameplan/tests/test_community_migrations.py` and will run once the bench's server-script preload issue is resolved (Phase 09 / CI).
- **`bench migrate` run twice on `gameplan.frappe.test` and `gameplan-demo.test`** — both patches execute cleanly on the first run and the second run is a clean no-op (no errors).
- **Cypress**: the previously-red onboarding + space-detail specs are now green. Root causes were spec-level, not just onboarding: (a) specs must `update_joined_teams` the seeded community (scoped guard 404s otherwise); (b) URL assertions used legacy unscoped `/g/space/...` paths — updated to canonical `/g/community/:communityId/space/...`; (c) `project.cy.js` "Change Category"→"Change Community" + "Select a community" rename, and the space-title breadcrumb is now a `<button>` (edit) not an `<a>` (the community name is the link); (d) `new-discussion.cy.ts` rewritten to use the scoped composer entry point (Phase 06 removed the `/drafts` "Add new" button); (e) `task.cy.js` projects were teamless — gave them a team.
- **Two small non-spec fixes were required for green:** `ChangeSpaceCategoryDialog.vue` now calls `spaces.reload()` after `move_to_team` so the breadcrumb's denormalized `team_title` updates; and a global `uncaught:exception` handler in `cypress/support/e2e.ts` ignores the benign `ResizeObserver loop` browser warning that was failing otherwise-passing specs.
- **`cy.scope('sidebar')` is unusable** — the `[data-sidebar]` selector it maps to does not exist in the app. Assert sidebar content with `cy.contains('a', 'SpaceName')` / `cy.contains('All Discussions')` instead (matches the other community specs).
